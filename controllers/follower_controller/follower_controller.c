...
void normalMode(){  
  ...
  diff = clock() - start;
  double msec = (double) (diff*1000/ CLOCKS_PER_SEC); // milliseconds

//Calculate the distance travelled
//double dis=sqrt(pow(position.x-x_initial,2)+pow(position.y-y_initial,2));           
if(msec >= nextSendingTime){
   printf("msec=%lf\n",msec);
   x_initial = wb_gps_get_values(gps)[X];
   y_initial = wb_gps_get_values(gps)[Z];  
   start_iteration=true;
   nextSendingTime = msec +5.0; // Send data after msec+5 milliseconds
   send_data_rear_vehicle(msec);
  }
...
void send_data_rear_vehicle(double msec){   
        double array[8] = {position.x,position.y,angle,steer,speed,current_speed,status,msec};
        wb_emitter_send(emitter_communication, array, 8* sizeof(double));
}
